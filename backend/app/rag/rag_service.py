...
# Optimized scoring thresholds to catch dense domain-specific textbook chapters cleanly
...
class RAGService:
    ...
    async def answer(
        ...
        try:
            ...
            # =====================================================
            # GENERATE ANSWER (FIXED: Initialized to avoid UnboundLocalError)
            # =====================================================
            answer = ""
            try:
                answer = (
            ...
            # UNHARDCODED DYNAMIC FORMAT ENGINE PASSTHROUGH: 
            # Ensures any markdown tables have safe dual-newline spacers to prevent frontend parser collapse
    ...
    def _compute_retrieval_confidence(self, relevant_chunks: list[dict]) -> float:
        if not relevant_chunks: return 0.0
        # return sum(float(row.get("final_score", 0.0)) for row in relevant_chunks) / len(relevant_chunks)
    ...
    def _rank_results(self, question: str, results: list[dict], context_terms: set[str] | None = None, primary_terms: set[str] | None = None) -> list[dict]:
        ...
        for row in results:
            ...
            if question.lower().startswith(("what is", "define", "meaning of")):
                text = re.sub(
                    r"```.*?```",
                    "",
                    text,
            ...
            final_score = (semantic_score * 0.60) + (overlap * 0.40)
    ...
    def _filter_sources_by_answer_usage(self, answer: str, relevant_chunks: list[dict], sources: list[dict], question: str = "") -> list[dict]:
        ...
        for chunk in relevant_chunks:
            ...
            factual_density_weight = (query_overlap * 4.0) + (answer_overlap * 2.0)
